...
var logger = require('log4js').getLogger("FetchResponse");
...
*/
var FetchResponse = function(head, topic, partition, error, highwaterMarkOffset, messages){
                       ...
                       .buffer('messageSet', 
                          response.body
                             .readUInt32BE(4 + 2 + topicNameLength
                                             + 4 + 4 + 2 + 8))
                       .vars;
  logger.debug("Get FetchResponse: " + JSON.stringify(unpacked));
  var messageSet = MessageSet.fromBytes(unpacked.messageSet);

  return new FetchResponse(response, unpacked.topic, unpacked.partition, unpacked.error, unpacked.highwaterMarkOffset, messageSet.messages);
};
