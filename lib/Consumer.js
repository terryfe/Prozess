...
utils.inherits(Consumer, event.EventEmitter);
...
Consumer.prototype.handleFetchData = function(cb){
  ...
  try {
    ...
    this.responseBuffer = this.responseBuffer.slice(response.head.bodyLength);
  } catch(ex){
  ...
  var messages = response.messageSet.messages;
  var messageLength = 0;
...
Consumer.prototype.sendConsumeRequest = function(cb){  
  ...
  this.onFetch(cb);
  var that = this;
  var fetchRequest =  new FetchRequest(this.topic, this.partition, this.offset, {});
