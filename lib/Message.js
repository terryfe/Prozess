...
Message.fromBytes = function(buf) {
  ...
  if (unpacked.length + 4 > buf.length || unpacked.key.length < keyLength) {    
    logger.debug('[Incomplete message] Need message length: ' + (unpacked.length + 4) + " Get: " + buf.length);
    logger.debug('                     Need key length: ' + keyLength + ' Get: ' + unpacked.key.length);
  ...
  if (unpacked.payload.length < payloadLength){
    logger.debug('[Incomplete message] Need payload length: ' + payloadLength + " Get: " + unpacked.payload.length);
    throw "incomplete message";
  }
